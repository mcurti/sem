...
f_s = 8.0;                                                                          % Fontsize
m_sl = 8.0;                                                                         % Markersize for legend
m_sp = 4.0;                                                                         % Markersize
m_so = 8.0;                                                                         % Markersize for optimum values
...
% FEM optimum
h_2 = loglog(N_f_F(I_FEM),e_FEM(I_FEM),'o','Color',[0 100 0]./256);
set(h_2,'MarkerSize',m_so);
set(h_2,'LineWidth',l_o);
% SEM pareto front
h_3 = loglog(P_f_S(:,1),P_f_S(:,2),'r-x');
set(h_3,'LineWidth',l_w);
% SEM data point
h_4 = loglog(NaN,NaN,'.','Color',[128 128 128]./256);
set(h_4,'MarkerSize',m_sl);
% SEM optimum
h_5 = loglog(N_f_s(I_SEM),e_SEM(I_SEM),'o','Color',[0 0 128]./256);
...
% Legend
[legend_h,object_h,plot_h,text_strings] = columnlegend(2,S,'Location','NorthOutside' ,'Fontsize',f_s,'Interpreter','LaTex');
...
% SEM data points
h_7 = loglog(N_f_s,e_SEM,'.','Color',[128 128 128]./256);
...
set(legend_h,'Position',[0.0955 0.6423 0.8440 0.35]);
set(gca,'Position',[0.1300    0.1200    0.7750    0.68]);
saveas(fig_1,'FEM_SEM_convergence','pdf')

%% Plot results normalized scale

fig_2 = figure(2);
% FEM pareto front
h_0 = plot(p_n_F(:,1),p_n_F(:,2),'k-x');
set(h_0,'LineWidth',l_w);
hold on;
% FEM data point
h_1 = plot(NaN,NaN,'.','Color',[192 192 192]./256);
set(h_1,'MarkerSize',m_sl);
% FEM optimum
h_2 = plot(f_n1_F(I_FEM),f_n2_F(I_FEM),'o','Color',[0 100 0]./256);
set(h_2,'MarkerSize',m_so);
set(h_2,'LineWidth',l_o);
% SEM pareto front
h_3 = plot(p_n_S(:,1),p_n_S(:,2),'r-x');
set(h_3,'LineWidth',l_w);
% SEM data point
h_4 = plot(NaN,NaN,'.','Color',[128 128 128]./256);
set(h_4,'MarkerSize',m_sl);
% SEM optimum
h_5 = plot(f_n1_S(I_SEM),f_n2_S(I_SEM),'o','Color',[0 0 128]./256);
set(h_5,'MarkerSize',m_so);
set(h_5,'LineWidth',l_o);
...
set(gca,'Ticklabelinterpreter','Latex','FontSize',f_s);
xlabel('$\bar{f}_n$ [-]','Interpreter','LaTex','FontSize',f_s);
ylabel('$\bar{f}_{\epsilon}$ [-]','Interpreter','LaTex','FontSize',f_s);
ylim([0 1])
xlim([0 1]);
%set(gca,'Xtick',[10^2 10^3 10^4]);
%set(gca,'Ytick',[10^(-5) 10^(-4) 10^(-3) 10^(-2) 10^(-1) 10^0 10^1 10^2]);
...
set(fig_2, 'Position',[25 10 width height],...
       'PaperSize',[width height],...
       'PaperPositionMode','auto',...
       'InvertHardcopy', 'off',...
       'Renderer','painters'...     %recommended if there are no alphamaps
...
figure(2);
% FEM data points
h_6 = plot(f_n1_F,f_n2_F,'.','Color',[192 192 192]./256);
set(h_6,'MarkerSize',m_sp);
% SEM data points
h_7 = plot(f_n1_S,f_n2_S,'.','Color',[128 128 128]./256);
set(h_7,'MarkerSize',m_sp);
...
%set(legend_h,'Position',[0.0955 0.6423 0.8440 0.35]);
%set(gca,'Position',[0.1300    0.1100    0.7750    0.68]);
saveas(fig_2,'FEM_SEM_pareto','pdf');
...
fig_3 = figure(3);
h_8 = semilogy(d_p_loop,e_p,'r-x');
%h_8 = loglog(d_p_loop,e_p,'r-x');
hold on;
set(h_8,'LineWidth',l_w);
grid on;
set(gca,'Ticklabelinterpreter','Latex','FontSize',f_s);
xlabel('$x_{6,m}$ [mm]','Interpreter','LaTex','FontSize',f_s);
ylabel('$\epsilon$ [\%]','Interpreter','LaTex','FontSize',f_s);
xlim([0 0.3]);
ylim([10^(-7) 10^2]);
h_9 = semilogy(d_p_loop(3),e_p(3),'o','Color',[0 0 128]./256);
