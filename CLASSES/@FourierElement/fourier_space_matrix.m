...
for k = 1:2
    if strcmp(type,'cartesian')
        ...
        if k == 1; dc = 1; else; dc = 0; end
        [h_p, h_n,  ~, ~] = hbnp_r(obj.w_n,obj.Edata.heights(k),...
            obj.ys(1,1),obj.ys(1,2));
...
<<<<<<< HEAD
            [ sin(xi_fourier{k}(1:end-1)'*obj.w_n')*diag(h_p), ...
...
=======
            [ sin(xi_fourier{k}(1:end-1)'*obj.w_n')*diag(h_p)*diag(obj.w_n), ...
            - sin(xi_fourier{k}(1:end-1)'*obj.w_n')*diag(h_n)*diag(obj.w_n), ...
              cos(xi_fourier{k}(1:end-1)'*obj.w_n')*diag(h_p)*diag(obj.w_n), ...
            - cos(xi_fourier{k}(1:end-1)'*obj.w_n')*diag(h_n)*diag(obj.w_n), ...
>>>>>>> d49a7cc26533bc656f3fa943d9eeae18b08a1501
               ones(numel(xi_fourier{k}(1:end-1)),1)./obj.Edata.heights(k), zeros(numel(xi_fourier{k}(1:end-1)),1)]*dc;
    end
